  ...
  <header class="py-3 flex justify-between items-center gap-2">
    ...
    <div class="flex shrink-0 p-1 bg-card-bg/50 border border-border rounded-lg" role="tablist">
      <button
        ...
        class="px-4 py-1.5 text-xs sm:text-sm font-bold rounded-md transition-all active:scale-98 {activeTab ===
        'camera'
          ? 'bg-app-bg text-primary shadow-sm border border-border/50'
          : 'text-text-muted border border-transparent hover:text-text-main'}"
      >
        Camera
      </button>
      <button
    ...
    <nav aria-label="Main Navigation" class="flex-1 flex justify-end">
      ...
      <div
        id="mobile-menu"
        class="absolute inset-0 z-40 bg-app-bg transition-transform duration-300 ease-out p-6 pt-[calc(var(--spacing-pwa-top)+4rem)] flex flex-col justify-between text-left {isMenuOpen
          ? 'translate-x-0'
          : '-translate-x-full'}"
      >
  ...
  >
    ...
    >
          ...
          >
            ...
            <div class="absolute bottom-4 inset-x-4 flex flex-col gap-2">
              ...
              >
                ...
                >
                  <option value="user" class="bg-neutral-900 text-white"
                    >Front Camera (Selfie)</option
                  >
      ...
      {:else}
        ...
        >
          ...
          />
          <div
            class="p-4 bg-app-bg rounded-full border border-border mb-4 group-hover:scale-105 transition-transform"
          >
            <svg
              class="w-8 h-8 text-text-muted group-hover:text-primary transition-colors"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
              viewBox="0 0 24 24"
    ...
    >
      ...
      <footer class="mt-auto pt-4">
        <button
          class="w-full bg-primary hover:bg-primary-hover text-black font-bold py-3.5 px-4 rounded-xl shadow-sm transition-colors cursor-pointer active:scale-[0.99]"
        >
          Process Frame
        </button>
      </footer>
